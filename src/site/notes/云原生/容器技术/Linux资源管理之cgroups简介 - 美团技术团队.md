---
{"dg-publish":true,"page-title":"Linux资源管理之cgroups简介 - 美团技术团队","url":"https://tech.meituan.com/2015/03/31/cgroups.html","tags":null,"permalink":"/云原生/容器技术/Linux资源管理之cgroups简介 - 美团技术团队/","dgPassFrontmatter":true}
---
...
转载自：https://tech.meituan.com/2015/03/31/cgroups.html
...
这里面两个函数分别代表安装和卸载某一个 cgroup 文件系统所需要执行的函数。每次把某一个 cgroups 子系统安装到某一个装载点的时候，cgroup\_mount 方法就会被调用，这个方法会生成一个 cgroups\_root（cgroups层级结构的根）并封装成超级快对象。
...
这里只有部分函数的实现，这是因为对于特定的文件系统而言，所支持的操作可能仅是 super\_operations 中所定义操作的一个子集，比如说对于块设备上的文件对象，肯定是支持类似 fseek 的查找某个位置的操作，但是对于 socket 或者 cgroups 这样特殊的文件系统，就不支持这样的操作。
...
比如挂载 cpuset, cpu, cpuacct, memory 4个subsystem到/cgroup/cpu\_and\_mem 目录下，就可以使用 `mount -t cgroup -o remount,cpu,cpuset,memory cpu_and_mem /cgroup/cpu_and_mem`
...
1 cgroups 详解：[http://files.cnblogs.com/files/lisperl/cgroups%E4%BB%8B%E7%BB%8D.pdf](http://files.cnblogs.com/files/lisperl/cgroups%E4%BB%8B%E7%BB%8D.pdf) 2 how to use cgroup: [http://tiewei.github.io/devops/howto-use-cgroup/](http://tiewei.github.io/devops/howto-use-cgroup/) 3 Control groups, part 6: A look under the hood: [http://lwn.net/Articles/606925/](http://lwn.net/Articles/606925/)