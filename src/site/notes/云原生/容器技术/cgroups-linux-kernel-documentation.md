...
"no internal process constraint" 这个术语在 cgroup（控制组）系统中指的是一条规则，该规则阻止了一个 cgroup（如果它不是层次结构中的叶节点）拥有进程。换句话说，在某些配置中，只有层次结构中的叶节点（或最终节点）可以与它们相关联的任务（进程）关联起来。这一限制确保了非叶节点纯粹作为组织或资源分配点，它们自身并不直接执行任何进程。
